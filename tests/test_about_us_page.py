...
def test_check_block_1(setup_browser, request):
    ...
    with allure.step('Проверка 1 блока'):
        blocs = page.query_selector_all('//*[@id="___gatsby"]/div[1]/section/div/div[2]/div/p')
        assert blocs[0].inner_text() == "We - we roast coffee. Craft of coffee roasting " \
                                        "is what we know best. We demand the same levels " \
                                        "of commitment to know-how from all of the people " \
                                        "we work with. The result of this mutual understanding " \
                                        "is a full-flavoured, balanced, and easily recognizable " \
                                        "taste of our coffees. We source and buy green coffees " \
...
def test_check_block_2(setup_browser, request):
    ...
    with allure.step('Проверка 2 блока'):
        ...
        assert blocs[1].inner_text() == "We love coffee. Coffee bean is fascinating. " \
                                        "There is nothing that simple and yet that difficult " \
                                        "at the same time. Coffee is a multicultural phenomenon, " \
                                        "a language understandable by most people on earth. " \
                                        "Coffee is a good with a very complex character, " \
                                        "multi-faceted, unpredictable. Coffee opens up only " \
                                        "to those who devote to it a lot of time and energy, " \
...
def test_check_block_3(setup_browser, request):
    ...
    with allure.step('Проверка 3 блока'):
        ...
        assert blocs[2].inner_text() == "In specialty coffee culture an ability to " \
                                        ...
                                        "In one cup of coffee we bring together efforts " \
                                        "of all of those who worked on the taste of that cup. " \
                                        "Efforts by farmers, pickers, those who processed, " \
                                        "sorted and graded the coffee, cuppers, roasters.", \
...
def test_check_block_5(setup_browser, request):
    ...
    with allure.step('Проверка 5 блока'):
        ...
        assert blocs[4].inner_text() == 'Strive for learning new things creates ' \
                                        ...
                                        'Beauty of coffee is that it is an inexhaustible ' \
                                        'source of knowledge for all of those who engage ' \
                                        'with this product. Our daily work with green and ' \
                                        'roasted coffee provides us with experience and ' \
                                        'encourages us to constantly search and gather ' \
                                        'more knowledge from coffee growers, baristas and ' \
                                        'other roasters. All the knowledge we accumulate ' \
